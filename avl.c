...
static struct node *
remove_node(struct avl *avl, struct node *root, const char *item, const char *word)
{
	int cmp;
	int balance_factor;
    if (!root) {
	...
	if (strcmp(word, root->item)==0) {
		avl->state->items -= root->count;
		avl->state->unique -= 1;
	}

    if (cmp < 0) {
        root->left = remove_node(avl, root->left, item, word);
    } else if (cmp > 0) {
        root->right = remove_node(avl, root->right, item, word);
    } else {
        if (!root->left && !root->right) {
            ...
            root->right = remove_node(avl, root->right, temp->item, word);
		}
		else{
            ...
            root->left = remove_node(avl, root->left, temp->item, word);
		}
    }
...
void avl_remove(struct avl *avl, const char *item)
{
    ...
    avl->state->root = remove_node(avl, avl->state->root, item, item);
}
